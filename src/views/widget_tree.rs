...
use uuid::Uuid;
use widgets::tree::{Branch, DropInfo, DropPosition, branch, tree_handle};
    ...
    match message {
        Message::TreeMove(drop_info) => {
            if let Some(target_external_id) = drop_info.target_id {
                ...
                let dragged_ids: Vec<WidgetId> = drop_info
                    .dragged_ids
                    .iter()
                    .map(|&id| WidgetId(id))
                    .collect();
                    ...
                    DropPosition::Into => {
                        ...
                        for dragged_id in dragged_ids {
                            if dragged_id != target_id {
                                ...
                                let target_child_count = hierarchy
                                    .get_widget_by_id(target_id)
                    ...
                    DropPosition::Before | DropPosition::After => {
                        if let Some(parent_id) = hierarchy.find_parent_id(target_id) {
                            for dragged_id in dragged_ids {
                                ...
                                let target_index = hierarchy
                                    .get_widget_by_id(parent_id)
                                    .and_then(|parent| {
                                        parent.children.iter().position(|c| c.id == target_id)
                                    })
                                    .unwrap_or(0);
        ...
        Message::SelectWidgets(external_ids) => {
            let widget_ids: HashSet<WidgetId> =
                external_ids.iter().map(|&id| WidgetId(id)).collect();
                    ...
                    if widget.widget_type == WidgetType::Space {
                        match change {
                            PropertyChange::Orientation(Orientation::Horizontal) => {
                                hierarchy.apply_property_change(
                                    id,
...
pub fn view<'a>(
    ...
    let overlay_content = build_editor_for_widget(
        hierarchy,
        type_system,
        theme,
        widget,
    ...
    let swap_label: Option<iced::advanced::widget::Text<'_, Theme, iced::Renderer>> =
        match widget.widget_type {
            WidgetType::Row => Some(icon_lucide::arrow_right_left()),
            WidgetType::Column => Some(icon_lucide::arrow_right_left()),
            WidgetType::Container => Some(icon_lucide::arrow_right_left()),
            WidgetType::Scrollable => Some(icon_lucide::arrow_right_left()),
            _ => None,
        };
    ...
    let disabled_delete_button: Element<Message> = {
    ...
    let root = branch(
        row![
            ...
            disabled_delete_button //place_holder
        ]
        .spacing(5),
    )
    .block_dragging()
    ...
    let mut tree = tree_handle(vec![root])
...
fn build_tree_item<'a>(
    ...
    let is_first_child_of_root = hierarchy
        .root()
        .children
        .first()
    ...
    let delete_button: Option<Element<Message>> = if widget.id.0 != 0 {
        // Don't allow deleting root
        Some(
            button(icon_lucide::trash_2())
    ...
    let edit_button: Element<Message> = if selection_count == 1 {
        ...
        Some(button(text("Selected")).style(button::secondary)).into()
    } else {
        Some(button(text("Edit")).style(button::text)).into()
    };
    ...
    let branch = match widget.widget_type {
        ...
        _ => {
            ...
            if !is_first_child_of_root {
                branch(content).with_id(widget.id.0)
            } else {
                // Block dragging for first child of root
                branch(content).with_id(widget.id.0).block_dragging()
            }
        }
    };
...
fn build_editor_for_widget<'a>(
    ...
    let controls_view: Element<Message> = match widget.widget_type {
        WidgetType::Container => container_controls(
            &hierarchy,
            widget_id,
            theme,
            &type_system,
        ...
        WidgetType::Scrollable => scrollable_controls(
            &hierarchy,
            widget_id,
            theme,
            &type_system,
        ...
        WidgetType::Column => column_controls(
            &hierarchy,
            widget_id,
            theme,
            &type_system,
        ...
        WidgetType::Button => button_controls(
            &hierarchy,
            widget_id,
            theme,
            &type_system,
        ...
        WidgetType::TextInput => text_input_controls(
            &hierarchy,
            widget_id,
            theme,
            &type_system,
        ...
        WidgetType::Checkbox => checkbox_controls(
            &hierarchy,
            widget_id,
            theme,
            &type_system,
        ...
        WidgetType::Radio => radio_controls(
            &hierarchy,
            widget_id,
            theme,
            &type_system,
        ...
        WidgetType::Toggler => toggler_controls(
            &hierarchy,
            widget_id,
            theme,
            &type_system,
        ...
        WidgetType::PickList => picklist_controls(
            &hierarchy,
            widget_id,
            theme,
            &type_system,
        ...
        WidgetType::DatePicker => date_picker_controls(
            &hierarchy,
            widget_id,
            theme,
            &type_system,
        ...
        WidgetType::Slider => slider_controls(
            &hierarchy,
            widget_id,
            theme,
            &type_system,
        ...
        WidgetType::VerticalSlider => vertical_slider_controls(
            &hierarchy,
            widget_id,
            theme,
            &type_system,
        ...
        WidgetType::Space => space_controls(
            &hierarchy,
            widget_id,
            theme,
            &type_system,
        ...
        WidgetType::ProgressBar => progress_controls(
            &hierarchy,
            widget_id,
            theme,
            &type_system,
        ...
        WidgetType::Image => image_controls(
            &hierarchy,
            widget_id,
            theme,
            &type_system,
        ...
        WidgetType::Tooltip => tooltip_controls(
            &hierarchy,
            widget_id,
            theme,
            &type_system,
        ...
        WidgetType::ComboBox => combobox_controls(
            &hierarchy,
            widget_id,
            theme,
            &type_system,
        ...
        WidgetType::Markdown => markdown_controls(
            &hierarchy,
            widget_id,
            theme,
            &type_system,
        ...
        WidgetType::MouseArea => mousearea_controls(
            &hierarchy,
            widget_id,
            theme,
            &type_system,
        ...
        WidgetType::QRCode => qrcode_controls(
            &hierarchy,
            widget_id,
            theme,
            &type_system,
        ...
        WidgetType::Stack => stack_controls(
            &hierarchy,
            widget_id,
            theme,
            &type_system,
        ...
        WidgetType::Collapsible => collapsible_controls(
            &hierarchy,
            widget_id,
            theme,
            &type_system,
        ...
        WidgetType::CollapsibleGroup => collapsible_group_controls(
            &hierarchy,
            widget_id,
            theme,
            &type_system,
        ...
        WidgetType::GenericOverlay => generic_overlay_controls(
            &hierarchy,
            widget_id,
            theme,
            &type_system,
        ...
        WidgetType::Themer => themer_controls(
            &hierarchy,
            widget_id,
            theme,
            &type_system,
        ...
        WidgetType::Table => table_controls(
            &hierarchy,
            widget_id,
            theme,
            &type_system,
        ...
        WidgetType::ViewReference => view_reference_controls(
            &hierarchy,
            widget_id,
            theme,
            &type_system,
