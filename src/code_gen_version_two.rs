...
pub fn generate_button_style_code(
    ...
    use helpers::{format_color_with_source, format_radius, format_shadow};
...
/// Generate container style function code as a String (tree_sitter handles highlighting)
pub fn generate_container_style_code(
    ...
    // text_color field (wrapped in Some)
...
/// Generate checkbox style function code as a String (tree_sitter handles highlighting)
pub fn generate_checkbox_style_code(
    ...
    use helpers::{format_color_with_source, format_radius};
    ...
    // background field (not wrapped in Option for checkbox)
    ...
    // text_color field (wrapped in Some for checkbox)
    code.push_str("        text_color: Some(");
    code.push_str(&format_color_with_source(text_color, text_color_source));
    code.push_str("),\n");
...
/// Generate combo box style function code as a String (tree_sitter handles highlighting)
/// Produces two functions: one for input_style (text_input::Style) and one for menu_style (menu::Style)
pub fn generate_combo_box_style_code(
    ...
    // background field (reuse the same background as input)
    code.push_str("        background: Background::Color(");
    code.push_str(&format_color_with_source(background_color, background_color_source));
    code.push_str("),\n");
